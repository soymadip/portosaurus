...
function getCssVar(varName) {
  ...
  if (varCache.has(varName)) {
    DEBUG &&
      console.log(
        `[CSS-DEBUG] Using cached value for ${varName}: ${varCache.get(varName)}`,
      );
  ...
  for (const cssPath of cssFiles) {
    try {
      ...
      while ((match = regex.exec(cssContent)) !== null) {
        ...
        DEBUG &&
          console.log(
            `[CSS-DEBUG] Found match #${matchCount} for ${varName}: ${lastValue}`,
          );
      }
      ...
      if (matchCount > 0) {
        DEBUG &&
          console.log(
            `[CSS-DEBUG] Using last of ${matchCount} matches for ${varName}: ${lastValue}`,
          );
      }
      ...
      if (lastValue && lastValue.startsWith("var(")) {
        ...
        if (nestedMatch) {
          ...
          DEBUG &&
            console.log(
              `[CSS-DEBUG] Found nested variable in ${varName}: ${nestedVar}`,
            );
          ...
          try {
            ...
            DEBUG &&
              console.log(
                `[CSS-DEBUG] Resolved nested ${nestedVar} to: ${resolvedValue}`,
              );
            ...
            DEBUG &&
              console.log(
                `[CSS-DEBUG] Could not resolve nested variable: ${err.message}`,
              );
            throw new Error(
              `Failed to resolve nested variable ${nestedVar} in ${varName}: ${err.message}`,
            );
          }
        }
      }
      ...
      if (lastValue) {
        DEBUG &&
          console.log(
            `[CSS-DEBUG] Caching and returning value for ${varName}: ${lastValue}`,
          );
      ...
      DEBUG &&
        console.log(`[CSS-DEBUG] Error processing ${cssPath}: ${err.message}`);
